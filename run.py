...
with open('./ajusted_dict.pkl', 'rb') as f:
    data_dict_real = pickle.load(f)
...
if args.is_training:
    for industry in data_dict_real.keys():
        for ii in range(args.itr):
            ...
            exp = Exp(args,data_dict_real)  # set experiments
            print('>>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
            exp.train(setting, industry)

            print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
            exp.test(setting, industry)

            if args.do_predict:
                print('>>>>>>>predicting : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
                exp.predict(setting, industry, True)
    ...
    exp = Exp(args,data_dict_real)  # set experiments
    print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
    exp.test(setting,industry, test=1)
    torch.cuda.empty_cache()
